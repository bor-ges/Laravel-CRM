...
@section('content')
    <div class="row">
        <div class="h4">Banco de Propostas</div>
    ...
    <div class="row ">
        ...
        <div class="col-md-3 w-auto">
            <a href="{{route('proposta.create')}}" type="button" class="btn btn-success">Adicionar nova proposta</a>
        </div>
    ...
    <div class="row">
        <h5 class="mb-3 mt-3">Tabela de Propostas</h5>
    </div>
                ...
                <div class="card-body px-0 pt-0 pb-2">
                    <div class="table-responsive p-0">
                        <table class="table align-items-center mb-0">
                            ...
                            <tbody>
                            @forelse ($propostas as $proposta) <tr>
                                <td>
                                    <div class="d-flex px-2 py-1">
                                        <div class="d-flex flex-column justify-content-center">
                                            <h6 class="mb-0 text-sm">{{ $proposta->nome_proposta }}</h6> </div>
                                    </div>
                                </td>
                                <td>
                                    <p class="text-xs font-weight-bold mb-0">{{ $proposta->tipo_proposta }}</p> </td>
                                <td class="align-middle text-center">
                                    <span class="text-secondary text-xs font-weight-bold">{{ $proposta->data_proposta }}</span> </td>
                                <td class="align-middle">
                                    <a href="#" class="text-secondary font-weight-bold text-xs" data-bs-toggle="modal" data-bs-target="#modal-{{ $proposta->id }}">
                                        Informações
                                    </a>
                                </td>
                            </tr>

                            <div class="modal fade" id="modal-{{ $proposta->id }}" tabindex="-1" role="dialog" aria-labelledby="modal-title-{{ $proposta->id }}" aria-hidden="true">
                                <div class="modal-dialog modal-modal-dialog-centered modal-" role="document">
                                    <div class="modal-content">
                                        <div class="modal-header">
                                            <h6 class="modal-title" id="modal-title-{{ $proposta->id }}">Informações</h6>
                                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close">
                                                <span aria-hidden="true">×</span>
                                            </button>
                                        </div>
                                        <div class="modal-body">
                                            <p><strong>Nome:</strong> {{ $proposta->nome_proposta }}</p>
                                            <p><strong>Tipo:</strong> {{ $proposta->tipo_proposta }}</p>
                                            <p><strong>Data de Realização:</strong> {{ $proposta->data_proposta }}</p>
                                            <p><strong>Descrição:</strong> {{ $proposta->descricao }}</p> </div>
                                            <p><strong>Anexos:</strong> <a href="{{$proposta->arquivo_proposta}}"> Abrir PDF</a></p>
